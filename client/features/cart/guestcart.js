...
const GuestCart = () => {
  ...
  const handleQuantityDecrease = (product) => {
    let idx = cart.indexOf(product);
    if (cart[idx].cartQuantity > 1) cart[idx].cartQuantity--;
    window.localStorage.setItem("cart", JSON.stringify(cart));
    setRender(!render);
  };
      ...
      <div id="cart-items-container">
        {cart && cart.length ? (
          cart.map((product) => (
            ...
            >
              <img
                key={`cart-item-img-${product.id}`}
                className="cart-item-img"
                src={product.imageUrl}
              />
              <div
                key={`cart-item-name-${product.id}`}
                className="cart-item-name"
              >{`${product.name}`}</div>
              <div
                key={`cart-item-price-${product.id}`}
                className="cart-item-price"
              >{`$${product.price}`}</div>
              <div
                key={`cart-item-quantity-${product.id}`}
                className="cart-quantity-container"
              >
                <img
                  onClick={(e) => handleQuantityIncrease(product)}
                  src="blackplus.png"
                  height="16px"
                />
                <div className="cart-item-total">{product.cartQuantity}</div>
                <img
                  onClick={(e) => handleQuantityDecrease(product)}
                  src="blackminus.png"
                  height="16px"
                />
              </div>
              ...
              <div
                id={`cart-item-total-${product.id}`}
                className="cart-item-price"
              >{`$${(product.cartQuantity * product.price).toFixed(2)}`}</div>
              <img
                onClick={(e) => handleItemDelete(product)}
                id={`cart-item-delete-${product}`}
                src="reddelete.png"
              />
            </div>
          ))
